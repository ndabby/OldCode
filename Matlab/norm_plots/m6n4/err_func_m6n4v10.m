function err_func = err_func_m6n4v10(input)
...
%(4,8) data
t = data(index_offset10:datasize,1)-offset10;  %what is 23 is the data point where offset exists



y0 = [start_conc10*(max10b-baseline10)/(max10-baseline10), 0, 0, k];

[t, y2] = ode45(@fmin_toehold_norm_10, t, y0, options);


ye = y2(:,3);

for i = index_offset10:(end_index - index_offset10) %cutting off the high rise at the end 

   
err_func = err_func + ((ye(i-(index_offset10 -1)) - (data(i, 2) - baseline10)/(max10-baseline10)*start_conc10)^2)*1e17; %min square difference of sim - data
