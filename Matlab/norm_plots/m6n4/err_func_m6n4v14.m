function err_func = err_func_m6n4v14(input)
...
%(4,8) data
t = data(index_offset14:datasize,1)-offset14;  %what is 23 is the data point where offset exists



y0 = [start_conc14*(max14b-baseline14)/(max14-baseline14), 0, 0, k];

[t, y2] = ode45(@fmin_toehold_norm_14, t, y0, options);


ye = y2(:,3);

for i = index_offset14:(end_index - index_offset14) %cutting off the high rise at the end 

   
err_func = err_func + ((ye(i-(index_offset14 -1)) - (data(i, 2) - baseline14)/(max14-baseline14)*start_conc14)^2)*1e17; %min square difference of sim - data
