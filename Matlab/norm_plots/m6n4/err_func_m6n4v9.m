function err_func = err_func_m6n4v9(input)
...
%(4,8) data
t = data(index_offset9:datasize,1)-offset9;  %what is 23 is the data point where offset exists



y0 = [start_conc9*(max9b-baseline9)/(max9-baseline9), 0, 0, k];

[t, y2] = ode45(@fmin_toehold_norm_9, t, y0, options);


ye = y2(:,3);

for i = index_offset9:(end_index - index_offset9) %cutting off the high rise at the end 

   
err_func = err_func + ((ye(i-(index_offset9 -1)) - (data(i, 2) - baseline9)/(max9-baseline9)*start_conc9)^2)*1e17; %min square difference of sim - data
