function err_func = err_func_m6n4v12(input)
...
%(4,8) data
t = data(index_offset12:datasize,1)-offset12;  %what is 23 is the data point where offset exists



y0 = [start_conc12*(max12b-baseline12)/(max12-baseline12), 0, 0, k];

[t, y2] = ode45(@fmin_toehold_norm_12, t, y0, options);


ye = y2(:,3);

for i = index_offset12:(end_index - index_offset12) %cutting off the high rise at the end 

   
err_func = err_func + ((ye(i-(index_offset12 -1)) - (data(i, 2) - baseline12)/(max12-baseline12)*start_conc12)^2)*1e17; %min square difference of sim - data
