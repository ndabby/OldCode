function err_func = err_func_m6n4v16(input)
...
%(4,8) data
t = data(index_offset16:datasize,1)-offset16;  %what is 23 is the data point where offset exists



y0 = [start_conc16*(max16b-baseline16)/(max16-baseline16), 0, 0, k];

[t, y2] = ode45(@fmin_toehold_norm_16, t, y0, options);


ye = y2(:,3);

for i = index_offset16:(end_index - index_offset16) %cutting off the high rise at the end 

   
err_func = err_func + ((ye(i-(index_offset16 -1)) - (data(i, 2) - baseline16)/(max16-baseline16)*start_conc16)^2)*1e17; %min square difference of sim - data
