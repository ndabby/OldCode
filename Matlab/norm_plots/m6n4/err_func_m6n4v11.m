function err_func = err_func_m6n4v11(input)
...
%(4,8) data
t = data(index_offset11:datasize,1)-offset11;  %what is 23 is the data point where offset exists



y0 = [start_conc11*(max11b-baseline11)/(max11-baseline11), 0, 0, k];

[t, y2] = ode45(@fmin_toehold_norm_11, t, y0, options);


ye = y2(:,3);

for i = index_offset11:(end_index - index_offset11) %cutting off the high rise at the end 

   
err_func = err_func + ((ye(i-(index_offset11 -1)) - (data(i, 2) - baseline11)/(max11-baseline11)*start_conc11)^2)*1e17; %min square difference of sim - data
