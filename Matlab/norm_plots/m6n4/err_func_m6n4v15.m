function err_func = err_func_m6n4v15(input)
...
%(4,8) data
t = data(index_offset15:datasize,1)-offset15;  %what is 23 is the data point where offset exists



y0 = [start_conc15*(max15b-baseline15)/(max15-baseline15), 0, 0, k];

[t, y2] = ode45(@fmin_toehold_norm_15, t, y0, options);


ye = y2(:,3);

for i = index_offset15:(end_index - index_offset15) %cutting off the high rise at the end 

   
err_func = err_func + ((ye(i-(index_offset15 -1)) - (data(i, 2) - baseline15)/(max15-baseline15)*start_conc15)^2)*1e17; %min square difference of sim - data
