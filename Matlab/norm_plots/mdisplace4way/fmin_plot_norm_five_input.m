...
k0 = log(1000000); %in this case (long toehold) we are using k1 as fixed and k2 as adjustable parameter

[estimated_k] = fminunc(@err_func_mdisplace4wayv1, k0);
...
[estimated_k] = fminunc(@err_func_mdisplace4wayv2, k0);
...
%[estimated_k] = fminunc(@err_func_mdisplace4wayv3, k0);
...
[estimated_k] = fminunc(@err_func_mdisplace4wayv4, k0);
...
[estimated_k] = fminunc(@err_func_mdisplace4wayv5, k0);
...
legend([p1 p2 p4 p5],'C = 50 nM, R = 10 nM','C = 5 nM, R = 2.5 nM', 'C = 5 nM, R = 2.5 nM', 'C = 5 nM, R = 1 nM', 'Location','Best');
set(gca,'FontSize',14);
xlabel('Time (in hours)','FontSize',16);
ylabel('Normalized Fluorescence (nM)', 'FontSize', 16);
title('mdisplace with n toehold present mean squared error fit for k1', 'FontSize', 16);