...
average(1) = (mean1(1)/(max1-baseline1))/(3*10^-6); %exp(estimated_k)
...
average(2) = (mean2(1)/(max2-baseline2))/(3*10^-6); %exp(estimated_k)
...
average(3) = (mean3(1)/(max3-baseline3))/(3*10^-6) %exp(estimated_k)
...
title('m2n0 plot mean squared error linear fit for k1', 'FontSize', 16);