...
k0 = log(0.05); %in this case (long toehold) we are using k1 as fixed and k2 as adjustable parameter

[estimated_k] = fminunc(@err_func_m2n0v1, k0);
...
[estimated_k] = fminunc(@err_func_m2n0v2, k0);
...
[estimated_k] = fminunc(@err_func_m2n0v3, k0);
...
legend([p1 p2 p3 p3c],'C = 3 uM, R = 120 nM', 'C = 3 uM, R = 120 nM', 'C = 3 uM, R = 250 nM', 'average fit', 'Location','Best')
set(gca,'FontSize',14);
xlabel('Time (in hours)','FontSize',16);
ylabel('Normalized Fluorescence (nM)', 'FontSize', 16);
title('m2n0 plot mean squared error fit for k1', 'FontSize', 16);