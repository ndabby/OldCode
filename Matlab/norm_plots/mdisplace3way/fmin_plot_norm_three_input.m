...
[estimated_k] = fminunc(@err_func_mdisplace3wayv1, k0);
...
[estimated_k] = fminunc(@err_func_mdisplace3wayv2, k0);
...
[estimated_k] = fminunc(@err_func_mdisplace3wayv3, k0);
...
legend([p1 p2 p3],'C = 50 nM, R = 10 nM','C = 5 nM, R = 2.5 nM', 'C = 5 nM, R = 1 nM','Location','Best');
set(gca,'FontSize',14);
xlabel('Time (in hours)','FontSize',16);
ylabel('Normalized Fluorescence (nM)', 'FontSize', 16);
title('mdisplace clean plot mean squared error fit for k1', 'FontSize', 16);