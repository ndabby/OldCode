...
k0 = 0.05 %log(0.05); %in this case (long toehold) we are using k1 as fixed and k2 as adjustable parameter

[estimated_k] = fminunc(@err_func_m0n0v1, k0);
...
average(1) = estimated_k; %exp(estimated_k)
...
[estimated_k] = fminunc(@err_func_m0n0v2, k0);
...
[estimated_k] = fminunc(@err_func_m0n0v3, k0);
...
legend([p1 p2 p3],'C = 3 uM, R = 120 nM', 'C = 3 uM, R = 120 nM', 'C = 3 uM, R = 250 nM', 'Location','Best')
set(gca,'FontSize',14);
xlabel('Time (in hours)','FontSize',16);
ylabel('Normalized Fluorescence (nM)', 'FontSize', 16);
title('m0n0 plot mean squared error fit for k1', 'FontSize', 16);