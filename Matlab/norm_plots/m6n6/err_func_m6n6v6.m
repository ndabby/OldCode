function err_func = err_func_m6n6v6(input)
...
%(4,8) data
t = data(index_offset6:datasize,1)-offset6;  %what is 23 is the data point where offset exists



y0 = [start_conc6*(max6b-baseline6)/(max6-baseline6), 0, 0, k];

[t, y2] = ode45(@fmin_toehold_norm_6, t, y0, options);


ye = y2(:,3);

for i = index_offset6:(end_index - index_offset6) %cutting off the high rise at the end 

   
err_func = err_func + ((ye(i-(index_offset6 -1)) - (data(i, 2) - baseline6)/(max6-baseline6)*start_conc6)^2)*1e17; %min square difference of sim - data
