...
void myfile::evictFileIO(int x){		//This function takes an integer argument
	tburstsofar = 0;					//and updates all relevent file info
	tsofar = tsofar + x;				//when an I/O request is signaled
	cputimer = cputimer + x;
	float temp = tnext;
	tnext = (alpha * x) + ((1 - alpha) * tlast);
	tlast = temp;	
	tremaining = tnext - tburstsofar;
};


void myfile::iorequest(string t, string n, string a, unsigned int l){
