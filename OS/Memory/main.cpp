...
int main(){
	...
	cout << "Welcome to the OS Installer!" <<endl;
	...
	while (memsize <= 0){
		cout << "Please enter the size of your memory in words (integer > 0)" <<endl;
		cin >> input;
		memsize = myatoi(input);
		if (memsize < 0){
			cout << "this is not a valid number." << endl;
		}
	}

	ndsystem sys(pnumber, dnumber, cnumber, alf, memsize); //system is generated
	...
	while (x < 0){
		...
		case 'A':								//process arrival
			...
			number = -1;
			while (number <= 0){
				cout << "How big is this process in words? (enter an integer > 0)" <<endl;
				cin >> prenum;
			...
			else { cout << "ERROR: process is too large for RAM!" << endl;}
			break;
