			...
			if (bot[g] == bot[h]){
				for( int i = 0; i < bot[g]->getTotLeg(); i++){
					for( int j = i + 1; j < bot[h]->getTotLeg(); j++){		
						...
						if ((distance(bot[g]->getXLocation(), bot[g]->getYLocation(), bot[h]->getXLocation(), bot[h]->getYLocation()) - 2*radius) <= 3){
...
void redraw(void)
{
			...
			while( y < ruleSize && yes == 0){
				...
				if (decidetable[z].kind1 == ruletable[y].lega1kind && decidetable[z].kind2 == ruletable[y].lega2kind
					...
					&& decidetable[z].bound == ruletable[y].bounda){
						...
						//motion
						if(ruletable[y].bounda == 0 && ruletable[y].boundb == 1 ){
							xlocal = 0.5*(bot[decidetable[z].bot1]->getXLocation() + bot[decidetable[z].bot2]->getXLocation());
							ylocal = 0.5*(bot[decidetable[z].bot1]->getYLocation() + bot[decidetable[z].bot2]->getYLocation());
							if(bot[decidetable[z].bot1]->getXLocation() < xlocal){
								bot[decidetable[z].bot1]->setX(xlocal - radius);
								bot[decidetable[z].bot2]->setX(xlocal + radius);						
							/**/	for(int g = 0; g < bot[decidetable[z].bot1]->getTotLeg(); g++){
									if (bot[decidetable[z].bot1]->getBound(g) != NULL && 
									    bot[decidetable[z].bot1]->getBound(g) !=bot[decidetable[z].bot2]){
											bot[decidetable[z].bot1]->getBound(g)->setX(xlocal - 3*radius);
										}
								}
								for(int h = 0; h < bot[decidetable[z].bot2]->getTotLeg(); h++){
									if (bot[decidetable[z].bot2]->getBound(h) != NULL && 
									    bot[decidetable[z].bot2]->getBound(h) != bot[decidetable[z].bot1]){
											bot[decidetable[z].bot2]->getBound(h)->setX(xlocal + 3*radius);
										}
								}	
							}
							else if(bot[decidetable[z].bot1]->getXLocation() > xlocal){
								bot[decidetable[z].bot1]->setX(xlocal + radius);
								bot[decidetable[z].bot2]->setX(xlocal - radius);
						/*	*/	for(int m = 0; m < bot[decidetable[z].bot1]->getTotLeg(); m++){
									if (bot[decidetable[z].bot1]->getBound(m) != NULL && 
									    bot[decidetable[z].bot1]->getBound(m) !=bot[decidetable[z].bot2]){
											bot[decidetable[z].bot1]->getBound(m)->setX(xlocal + 3*radius);
										}
								}
								for(int n = 0; n < bot[decidetable[z].bot2]->getTotLeg(); n++){
									if (bot[decidetable[z].bot2]->getBound(n) != NULL && 
									    bot[decidetable[z].bot2]->getBound(n) !=bot[decidetable[z].bot1]){
											bot[decidetable[z].bot2]->getBound(n)->setX(xlocal - 3*radius);
										}
								}	
							}		
						}
	...
	for(int q = 0; q< decSize; q++){
		...
		if ((distance(bot[decidetable[q].bot1]->getXLocation(), bot[decidetable[q].bot1]->getYLocation(), 
			bot[decidetable[q].bot2]->getXLocation(), bot[decidetable[q].bot2]->getYLocation()) - 2*radius) <= 3){
